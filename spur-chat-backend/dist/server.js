...
app.post('/api/message', async (req, res) => {
    try {
        ...
        const saveUserMsg = db.prepare('INSERT INTO messages (id, conversation_id, sender, text) VALUES (?, ?, ?, ?)');
        saveUserMsg.run(userMsgId, convId, 'user', userMessage);
        // Get conversation history (last 6 messages for context)
        const history = db.prepare(`SELECT sender, text 
       FROM messages 
       WHERE conversation_id = ? 
       ORDER BY created_at DESC 
       LIMIT 6`).all(convId).reverse();
        ...
        const saveAiMsg = db.prepare('INSERT INTO messages (id, conversation_id, sender, text) VALUES (?, ?, ?, ?)');
...
app.get('/api/conversation/:id', (req, res) => {
    try {
        ...
        const messages = db.prepare(`SELECT id, sender, text, created_at 
       FROM messages 
       WHERE conversation_id = ? 
...
app.delete('/api/conversation/:id', (req, res) => {
    try {
        ...
        console.log(`Deleted conversation ${id}, leftover messages: ${(leftover?.cnt) || 0}`);
...
app.get('/api/conversations', (req, res) => {
    try {
        ...
        const rows = db.prepare('SELECT id, created_at FROM conversations ORDER BY created_at DESC').all();
        const conversations = rows.map(r => {
            // small extra query per row - could be optimized with a JOIN in prod
            const cntRow = db.prepare('SELECT COUNT(*) as cnt FROM messages WHERE conversation_id = ?').get(r.id);
            return {
                id: r.id,
